...
#import "YHKitMacro.h"
#import "NSArray+YHAdd.h"
#import "UIDevice+YHAdd.h"
#import "NSObject+YHAdd.h"
#import <sys/sysctl.h>
#import <mach/mach.h>
#import <objc/runtime.h>

YHSYNTH_DUMMY_CLASS(UIApplication_YHAdd)

#define kNetworkIndicatorDelay (1 / 30.0)

@interface _YHUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _YHUIApplicationNetworkIndicatorInfo
@end
...
- (BOOL)isPirated {
    ...
    if ([[[NSBundle mainBundle] infoDictionary] objectForKey:@"SingerIdentity"]) {
        return YES;
    }
    
    if (![self _yh_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _yh_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_yh_fileExistInMainBundle:(NSString *)name {
...
YHSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _YHUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkAvtivityCount:(NSInteger)delta {
    @synchronized(self) {
        dispatch_async_on_main_queue(^{
            _YHUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_YHUIApplicationNetworkIndicatorInfo new];
...
- (void)incrementNetworkActivityCount {
    [self _changeNetworkAvtivityCount:1];
}
